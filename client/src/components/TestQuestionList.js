...
const TestQuestionList =  observer(() => {
    ...
    const questionHandler = (id, right) => {
        setAnswers(answer => {answer[id]= right; return answer})
    ...
    const addDocument = async () => {
        await addDoc(collection(firestore, "testResult"), {user:currentUserRef, test:currentTestRef, answers:answers});
      };
        ...
        <Row className="d-flex">
            {testQuestions?.docs.map((testQuestion, index) => 
                 <div className="d-flex">
                    <TestQuestionItem key={testQuestion.id} testQuestionSnap={testQuestion} handler={questionHandler} index={index} /> 
                 </div>
            )}
